...
def test_recognize_activity_walking():
    ...
    accel_data = create_test_acceleration_data(activity_type="walking", duration_seconds=30)
...
def test_recognize_activity_sitting():
    ...
    accel_data = create_test_acceleration_data(activity_type="sitting", duration_seconds=30)
    ...
    found_sitting = False
    for segment in data["activity_segments"]:
        if segment["activity_type"] == ActivityType.SITTING or segment["activity_type"] == ActivityType.STANDING:
            found_sitting = True
            break
...
def test_recognize_activity_mixed():
    ...
    walking_data = create_test_acceleration_data(activity_type="walking", duration_seconds=15)
    
    # Create second half with sitting data
    sitting_data = create_test_acceleration_data(activity_type="sitting", duration_seconds=15)
    
    # Combine the samples and adjust timestamps for the sitting data
    time_offset = walking_data.samples[-1].timestamp - sitting_data.samples[0].timestamp + timedelta(seconds=1)
    ...
    assert len(data["activity_patterns"]) > 0, "Should detect activity patterns with mixed data"

def test_calculate_activity_metrics():
    """Test the activity metrics calculation endpoint."""
    # Create test data
    accel_data = create_test_acceleration_data(activity_type="walking", duration_seconds=20)
...
def test_get_activity_types():
    ...
    expected_types = ["walking", "running", "standing", "sitting", "lying", "cycling", "unknown"]
    for activity_type in expected_types:
        assert activity_type in types, f"Activity type '{activity_type}' should be in the list"
