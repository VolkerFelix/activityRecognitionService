...
def extract_features(data: AccelerationData):
    ...
    for i in range(0, len(samples) - window_size + 1, window_size // 2):  # 50% overlap
        ...
        feature_vector = {
            ...
            "mean_mag": np.mean(
                [np.sqrt(s.x**2 + s.y**2 + s.z**2) for s in window]
            ),
...
def detect_activity_segments(data: AccelerationData) -> List[ActivitySegment]:
            ...
            if current_activity and segment_start:
                ...
                segment_data = AccelerationData(
                    ...
                    samples=[
                        ...
                        if segment_start <= s.timestamp <= features["start_time"]
                    ],
                )
    ...
    if current_activity and segment_start and segment_features:
        ...
        segment_data = AccelerationData(
            ...
            start_time=segment_start,
            samples=[
                s for s in data.samples if segment_start <= s.timestamp <= last_time
            ],
        )
...
def detect_activity_patterns(
    data: AccelerationData, segments: List[ActivitySegment]
    ...
    sedentary_segments = [
        ...
        for s in segments
        if s.activity_type
        in [ActivityType.SITTING, ActivityType.STANDING, ActivityType.LYING]
    ]

    if sedentary_segments:
        total_sedentary_duration = (
            sum(
                [
                    (s.end_time - s.start_time).total_seconds()
                    for s in sedentary_segments
                ]
            )
            / 60.0
        )  # Convert to minutes
    ...
    active_segments = [
        ...
        for s in segments
        if s.activity_type
        in [ActivityType.WALKING, ActivityType.RUNNING, ActivityType.CYCLING]
    ]

    if active_segments:
        total_active_duration = (
            sum([(s.end_time - s.start_time).total_seconds() for s in active_segments])
            / 60.0
        )  # Convert to minutes
        ...
        if unique_activities >= 3:  # At least 3 different activity types
            patterns.append(
                ActivityPattern(
                    ...
                    total_duration=sum(
                        [(s.end_time - s.start_time).total_seconds() for s in segments]
                    )
                    / 60.0,
                    segments=segments,
                )
